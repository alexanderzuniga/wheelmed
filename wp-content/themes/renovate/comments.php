		...
		<div class="comment-details">
			<div class="posted-by clearfix">
				...
				{	
					...
					echo '<a href="#comment-' . (int)esc_attr($comment->comment_parent) . '" class="in-reply">@' . $parent_author . '</a>';
				}
				?>
				</h6>
				<abbr title="<?php printf(esc_html__(' %1$s, %2$s', 'renovate'), get_comment_date(),  get_comment_time()); ?>" class="timeago"><?php printf(__(' %1$s, %2$s', 'renovate'), get_comment_date(),  get_comment_time()); ?></abbr>
				<a class="more simple small reply-button" href="#<?php echo esc_attr(get_comment_ID()); ?>" title="<?php esc_attr_e('Reply', 'renovate'); ?>"><?php _e('REPLY', 'renovate'); ?></a>
			</div>
			<?php 
			comment_text(); 
