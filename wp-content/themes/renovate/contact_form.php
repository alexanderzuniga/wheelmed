...
function re_theme_contact_form_shortcode($atts)
{
	...
	extract(shortcode_atts(array(
		"id" => "contact_form",
		"submit_label" => __("SEND MESSAGE", 'renovate'),
		"name_label" => __("Your Name *", 'renovate'),
		"name_required" => 1,
		"email_label" => __("Your Email *", 'renovate'),
		"email_required" => 1,
		"phone_label" => __("Your Phone", 'renovate'),
		"phone_required" => 0,
		"message_label" => __("Message *", 'renovate'),
		"message_required" => 1,
	...
	$output .= '<form class="contact-form ' . ($top_margin!="none" ? esc_attr($top_margin) : '') . ($el_class!="" ? ' ' . esc_attr($el_class) : '') . '" id="' . esc_attr($id) . '" method="post" action="#">
		<div class="vc_row wpb_row vc_inner">
			...
			<fieldset class="vc_col-sm-6 wpb_column vc_column_container">
				<div class="block">
					<textarea name="message" placeholder="' . esc_attr($message_label) . '" data-default="' . esc_attr($message_label) . '"' . ((int)$message_required ? ' data-required="1"' : '') . '>' . $message_label . '</textarea>
				</div>
			</fieldset>
		</div>
			...
			<input type="submit" name="submit" value="' . esc_attr($submit_label) . '" class="more">
				...
				if((int)$theme_options["google_recaptcha"])
				{
					if($theme_options["recaptcha_site_key"]!="" && $theme_options["recaptcha_secret_key"]!="")
					{
						...
						$output .= '<p>' . __("Error while loading reCapcha. Please set the reCaptcha keys under Theme Options in admin area", 'renovate') . '</p>';
					if((int)$terms_checkbox)
					{
						$output .= '</div>';
					}
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				...
				"value" => __("SEND MESSAGE", 'renovate')
			),
			array(
				"type" => "textfield",
				"class" => "",
				"heading" => __("Name label", 'renovate'),
				"param_name" => "name_label",
				"value" => __("Your Name *", 'renovate')
			),
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Name field required", 'renovate'),
			...
			array(
				"type" => "textfield",
				"class" => "",
				"heading" => __("Email label", 'renovate'),
				"param_name" => "email_label",
				"value" => __("Your Email *", 'renovate')
			),
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Email field required", 'renovate'),
			...
			array(
				"type" => "textfield",
				"class" => "",
				"heading" => __("Phone label", 'renovate'),
				"param_name" => "phone_label",
				"value" => __("Your Phone", 'renovate')
			),
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Phone field required", 'renovate'),
			...
			array(
				"type" => "textfield",
				"class" => "",
				"heading" => __("Message label", 'renovate'),
				"param_name" => "message_label",
				"value" => __("Message *", 'renovate')
			),
			array(
				"type" => "dropdown",
				"class" => "",
				"heading" => __("Message field required", 'renovate'),
			...
			array(
				...
				"value" => ((int)$theme_options["google_recaptcha"] ? __("Yes", 'renovate') : __("No", 'renovate')),
	...
	if(((isset($_POST["terms"]) && (int)$_POST["terms"]) || !isset($_POST["terms"])) && (((int)$theme_options["google_recaptcha"] && !empty($_POST["g-recaptcha-response"])) || !(int)$theme_options["google_recaptcha"]) && ((isset($_POST["name_required"]) && (int)$_POST["name_required"] && $_POST["name"]!=$_POST["name_default"] && $_POST["name"]!="") || (!isset($_POST["name_required"]) || !(int)$_POST["name_required"])) && ((isset($_POST["email_required"]) && (int)$_POST["email_required"] && $_POST["email"]!=$_POST["email_default"] && $_POST["email"]!="" && preg_match("#^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,12})$#", $_POST["email"])) || (!isset($_POST["email_required"]) || !(int)$_POST["email_required"])) && ((isset($_POST["phone_required"]) && (int)$_POST["phone_required"] && $_POST["phone"]!=$_POST["phone_default"] && $_POST["phone"]!="") || (!isset($_POST["phone_required"]) || !(int)$_POST["phone_required"])) && ((isset($_POST["message_required"]) && (int)$_POST["message_required"] && $_POST["message"]!=$_POST["message_default"] && $_POST["message"]!="") || (!isset($_POST["message_required"]) || !(int)$_POST["message_required"])))
	{
		...
		if(((int)$theme_options["google_recaptcha"] && isset($verify_recaptcha["success"]) && (int)$verify_recaptcha["success"]) || !(int)$theme_options["google_recaptcha"])
		{
			...
			if(wp_mail($theme_options["cf_admin_name"] . ' <' . $theme_options["cf_admin_email"] . '>', $subject, $body, $headers))
				$result["submit_message"] = (!empty($theme_options["cf_thankyou_message"]) ? $theme_options["cf_thankyou_message"] : __("Thank you for contacting us", 'renovate'));
			else
			{
				$result["isOk"] = false;
				$result["error_message"] = $GLOBALS['phpmailer']->ErrorInfo;
				$result["submit_message"] = (!empty($theme_options["cf_error_message"]) ? $theme_options["cf_error_message"] : __("Sorry, we can't send this message", 'renovate'));
			}
		}
		else
		{
			$result["isOk"] = false;
			$result["error_captcha"] = (!empty($theme_options["cf_recaptcha_message"]) ? $theme_options["cf_recaptcha_message"] : __("Please verify captcha.", 'renovate'));
		}
	}
	else
	{
		...
		if(isset($_POST["name_required"]) && (int)$_POST["name_required"] && ($_POST["name"]==$_POST["name_default"] || $_POST["name"]==""))
			$result["error_name"] = (!empty($theme_options["cf_name_message"]) ? $theme_options["cf_name_message"] : __("Please enter your name.", 'renovate'));
		if(isset($_POST["email_required"]) && (int)$_POST["email_required"] && ($_POST["email"]==$_POST["email_default"] || $_POST["email"]=="" || !preg_match("#^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,12})$#", $_POST["email"])))
			$result["error_email"] = (!empty($theme_options["cf_email_message"]) ? $theme_options["cf_email_message"] : __("Please enter valid e-mail.", 'renovate'));
		if(isset($_POST["phone_required"]) && (int)$_POST["phone_required"] && ($_POST["phone"]==$_POST["phone_default"] || $_POST["phone"]==""))
			$result["error_phone"] = (!empty($theme_options["cf_phone_message"]) ? $theme_options["cf_phone_message"] : __("Please enter your phone number.", 'renovate'));
		if(isset($_POST["message_required"]) && (int)$_POST["message_required"] && ($_POST["message"]==$_POST["message_default"] || $_POST["message"]==""))
			$result["error_message"] = (!empty($theme_options["cf_message_message"]) ? $theme_options["cf_message_message"] : __("Please enter your message.", 'renovate'));
		if((int)$theme_options["google_recaptcha"] && empty($_POST["g-recaptcha-response"]))
			$result["error_captcha"] = (!empty($theme_options["cf_recaptcha_message"]) ? $theme_options["cf_recaptcha_message"] : __("Please verify captcha.", 'renovate'));
		if(isset($_POST["terms"]) && !(int)$_POST["terms"])
			$result["error_terms"] = (!empty($theme_options["cf_terms_message"]) ? $theme_options["cf_terms_message"] : __("Checkbox is required.", 'renovate'));
	}
